...
def make_waveform_thumb(wav_path: Path, png_path: Path, width=640, height=160):
    ...
    try:
        ...
        print("thumb read error:", e)
        return

    # downsample visually if very long
    if len(y) > 200_000:
...
def recent_gallery_data(max_items: int = 20):
    ...
    for wav in waves:
        ...
        try:
            ...
            continue
        items.append({"image": img, "label": wav.name, "metadata": {"path": str(wav.resolve())}})
    return items
...
def _play_recent_from_gallery(evt: gr.SelectData):
    """Load the selected gallery audio into the Output player."""
    val = evt.value
    path = None
    label = None
    if isinstance(val, dict):  # new gradio
        meta = val.get("metadata") or {}
        path = meta.get("path")
        label = val.get("label")
    elif isinstance(val, (list, tuple)) and len(val) >= 2:  # older gradio
        label = val[1]
    else:
        label = str(val) if val is not None else None

    audio_path = Path(path) if path else (OUT_DIR / (label or ""))
    if not audio_path.exists():
        raise gr.Error("Audio not found for the selected item.")
...
with gr.Blocks(title=APP_NAME, theme=gr.themes.Soft(), fill_height=True) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            # 2) A small manual refresh button (define it before .click)
            refresh_recent_btn = gr.Button("↻ Refresh recent", size="sm")

            # 3) Fill the gallery AFTER the UI mounts (prevents spinner-on-load)
            demo.load(lambda: recent_gallery_data(), inputs=None, outputs=recent)

            # 4) Click a thumbnail → play in Output
            recent.select(_play_recent_from_gallery, inputs=None, outputs=audio_out)

            # 5) Hook the refresh button
            refresh_recent_btn.click(fn=lambda: recent_gallery_data(), inputs=None, outputs=recent)

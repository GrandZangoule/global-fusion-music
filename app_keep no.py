...
# ---- helpers for thumbs and dataset ----
...
def recent_dataset_rows(max_items=20):
    """Each row: [abs wav path, abs png path] for the Dataset."""
    THUMBS.mkdir(parents=True, exist_ok=True)
    rows = []
    waves = sorted(OUT_DIR.glob("*.wav"),
                   key=lambda p: p.stat().st_mtime, reverse=True)[:max_items]
    for wav in waves:
        png = THUMBS / f"{wav.stem}.png"
        if not png.exists():
            try:
                make_waveform_thumb(wav, png)
            except Exception as e:
                print("thumb error:", e)
        rows.append([str(wav.resolve()), str(png.resolve())])
    return rows

def recent_gallery_data(max_items: int = 20):
    """
    Build data for gr.Gallery: a list of dicts like
    ...
    for wav in waves:
        thumb_path = _make_waveform_thumb(str(wav))
        if not thumb_path:
            continue
        try:
        ...
        items.append({
            "image": img,
            "label": wav.name,
            "metadata": {"path": str(wav)}
        })

    return items

def _read_audio_to_numpy(path: Path) -> tuple[int, np.ndarray]:
    """Read an audio file -> (sr, mono-f32 wav)."""
...
def _last_generated_numpy_and_thumb():
    """Newest .wav -> (sr, wav), preview thumb path (or None)."""
    waves = sorted(
        OUT_DIR.glob("*.wav"),
        key=lambda p: p.stat().st_mtime,
        reverse=True
    ...
    p = waves[0]
    sr, y = _read_audio_to_numpy(p)
    thumb_path = _make_waveform_thumb(str(p))
    return (sr, y), (thumb_path if thumb_path else None)

def _play_from_gallery_numpy(val):
    """
    Gallery select callback (we only react to double-click via _js gate in the binding).
    val can be either [image, label] or an object with .metadata.path depending on Gradio version.
    We resolve the .wav path and return (sr, wav) + preview image path.
    """
    # Try to get the label name
    label = None
    ...
    if isinstance(val, dict) and "metadata" in val and isinstance(val["metadata"], dict):
        meta_path = val["metadata"].get("path")

    if meta_path:
        p = Path(meta_path)
    elif label:
        # fall back to OUT_DIR/label
        p = OUT_DIR / label
    else:
...
def _play_recent_from_dataset_numpy(row, prev_idx: int, prev_ts: float, evt: gr.SelectData | None = None):
    """Double-click only: single click updates state; double click plays."""
    import time as _t
    if evt is None or getattr(evt, "index", None) is None or not row:
        return gr.update(), prev_idx, prev_ts
    idx = evt.index
    now = _t.time()
    if (prev_idx == idx) and ((now - prev_ts) <= DOUBLE_CLICK_WINDOW):
        wav_path = Path(row[0]) if isinstance(row, (list, tuple)) and len(row) > 0 else None
        if not wav_path or not wav_path.exists():
            return gr.update(), -1, 0.0
        wav, sr = sf.read(str(wav_path), dtype="float32", always_2d=False)
        if getattr(wav, "ndim", 1) == 2:
            wav = wav.mean(axis=1)
        return (sr, wav), -1, 0.0  # play + reset
    # single click → remember
...
def _play_any_audio_file(path_str: str):
    """Read any OS audio file and return (sr, wav) for gr.Audio(type='numpy')."""
    if not path_str:
        return gr.update()
    p = Path(path_str)
    if (not p.exists()) or (p.suffix.lower() not in AUDIO_EXTS):
        raise gr.Error("Please drop/select a real audio file (e.g., .wav).")
...
def generate_music(
    user_prompt: str,
    melody_file: tuple[int, np.ndarray] | None,
    duration_sec: int,
    top_k: int,
    temperature: float,
    cfg: float,
    ...
    try:
        ...
        pass  # some wrappers may not have .to()
    ...
    if use_melody:
        ...
        melody_wav = melody_wav.to(device, non_blocking=True)
    ...
    for i in range(state.next_chunk, n_chunks):
        ...
        # (status textbox, audio_out placeholder, file path placeholder)
    ...
    # Update/create gallery thumbnail
    _make_waveform_thumb(str(path))

    final_status = f"Done ✅ Saved {fname}"

    # ---- FINAL YIELD (same 3 outputs) ----
    # (status textbox, audio_out numpy tuple, file path string)
...
with gr.Blocks(title=APP_NAME, theme=gr.themes.Soft(), fill_height=True) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            # Output player (unchanged; accepts OS files)
            audio_out = gr.Audio(label="🔊 Output", type="numpy", interactive=True, sources=["upload"])

            # Optional: small OS file picker
            file_out = gr.File(
                label="Load an audio file from your computer (drag & drop .wav/.mp3/.flac/.ogg/.m4a)",
            ...
            # History table (your schema)
            hist = gr.Dataframe(
                headers=["time", "prompt", "file", "duration_s", "style", "rhythm", "key",
                        "bpm", "top_k", "temperature", "cfg", "seed"],
            ...
            # -------- Enlarged preview that persists (note type="filepath") --------
            preview_img = gr.Image(label="Preview", interactive=False, height=220, type="filepath")

            # -------- Recent (NO extra boxes; components defined inline here) -------
            recent = gr.Dataset(
                label="Recent (double-click to play; each row is .wav + thumbnail)",
                components=[
                    gr.File(show_label=False, interactive=False),                     # small file chip
                    gr.Image(show_label=False, interactive=False, height=80),         # tiny thumb
                ],
                headers=["file", "image"],
                samples=[],                 # filled on load/refresh
                samples_per_page=12,
            )
            ...
            # Load & refresh the dataset rows
            demo.load(lambda: gr.update(samples=recent_dataset_rows()), outputs=recent)
            refresh_recent_btn.click(lambda: gr.update(samples=recent_dataset_rows()), outputs=recent)

            # Last generated -> play
            last_btn.click(_last_generated_numpy, outputs=audio_out)

            # Double-click only to play; single click updates preview
            last_click_idx  = gr.State(-1)
            last_click_time = gr.State(0.0)

            def _select_recent(row, prev_idx: int, prev_ts: float, evt: gr.SelectData | None = None):
                """
                row = [abs_wav_path, abs_png_path]
                Single click: update preview only.
                Double click within DOUBLE_CLICK_WINDOW on same row: play audio.
                """
                import time as _t
                ...
                if row and isinstance(row, (list, tuple)) and len(row) > 1:
                    preview_path = str(row[1])  # filepath string for preview

                if evt is None or getattr(evt, "index", None) is None or not row:
                    return audio_update, preview_path, prev_idx, prev_ts
                ...
                if (prev_idx == idx) and ((now - prev_ts) <= DOUBLE_CLICK_WINDOW):
                    ...
                    if wav_path.exists():
                        ...
                        if getattr(wav, "ndim", 1) == 2:
                            wav = wav.mean(axis=1)
                        audio_update = (sr, wav)
                    return audio_update, preview_path, -1, 0.0  # reset the click state

                # Single click → preview only, remember click
                return audio_update, preview_path, idx, now

            recent.select(
                _select_recent,
                inputs=[last_click_idx, last_click_time],
                outputs=[audio_out, preview_img, last_click_idx, last_click_time],
            )
            

    # ---- State + Callbacks ----
    state_hist = gr.State([])  # list of rows (list-of-lists) for the Dataframe

    def on_preset(name: str):
        conf = PRESETS.get(name, {})
        # return updates in the same order as outputs below
        return (
    ...
    # Wire each preset button with its name (no inputs)
    for btn, name in zip(preset_btns, PRESETS.keys()):
        btn.click(
            fn=lambda _=None, n=name: on_preset(n),
            inputs=None,
            outputs=[style, rhythm, bpm, temperature, cfg, top_k],
        )

    # Helper to append a history row and update the table
    HIST_HEADERS = ["time","prompt","file","duration_s","style","rhythm",
                    "key","bpm","top_k","temperature","cfg","seed"]

    def _append_history_row(prompt_val, duration_val, topk_val, temp_val, cfg_val, bpm_val,
                            style_val, rhythm_val, keyroot_val, scale_val, seed_val,
                            hist_list, fpath):
        """Return (new_state_hist, Dataframe.update) where rows are list-of-lists."""
        ts = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
        row = [
            ts,
            str(prompt_val or ""),
            ...
            str(style_val),
            str(rhythm_val),
            f"{keyroot_val} {scale_val}",
            int(bpm_val),
            int(topk_val),
            float(temp_val),
            float(cfg_val),
            int(seed_val) if isinstance(seed_val, (int, float)) else 0,
        ]
        new_hist = (hist_list or []) + [row]
        return new_hist, gr.update(value=new_hist)

    # NOTE: generate_music streams THREE outputs:
    #   1) status_box (text)  2) audio_out (numpy tuple)  3) file_out (saved path str)
    generate_btn.click(
        fn=generate_music,
    ...
    clear_btn.click(do_clear, inputs=None, outputs=[state_hist, hist])
...
with gr.Accordion("🎛️ Mixtape Builder (sequence clips with crossfades)", open=False):
    ...
    # Outputs (hidden until we actually have a result)
    mix_status = gr.Textbox(label="Mix status", interactive=False)
    mix_audio  = gr.Audio(
        label="Mixtape (preview/download)",
        type="filepath",
        interactive=False,
        visible=False,   # hidden initially
    )
    mix_file   = gr.File(
        label="Final mixtape file (.wav)",
        visible=False,   # hidden initially
    )
    ...
    def build_mixtape_ui(rows, bpm_grid, beats):
        ...
        for r in (rows or []):
            ...
            segments.append(dict(
                user_prompt=str(r[0]),
                melody_file=None,                     # extend later for per-segment melody
        ...
        for i, seg in enumerate(segments, 1):
            # stream status; keep other outputs unchanged
            yield (gr.update(value=f"Rendering segment {i}/{total}…"),
                   gr.update(), gr.update())
            p = render_segment_to_path(**seg)  # <-- your existing function must return a wav path
            paths.append(p)

        # 3) Stitch with crossfades
        yield (gr.update(value="Stitching segments…"),
               gr.update(), gr.update())
        out_path = stitch_paths_to_wav(
            paths,
            bpm_for_xfade=float(bpm_grid),
            xfade_beats=float(beats),
        )  # <-- your existing function returns a final wav path

        # 4) Final: show + populate preview & file
        status_msg = f"Done ✅ Saved {os.path.basename(out_path)}"
        return (
            gr.update(value=status_msg),
            gr.update(value=str(out_path), visible=True),  # show Audio (filepath)
            gr.update(value=str(out_path), visible=True),  # show File
        )

    # Wire it up (streaming status while building)
    build_mix_btn.click(
        fn=build_mixtape_ui,
        ...
        fn=lambda: recent_gallery_data(),  # refresh your recent list after new file lands
        inputs=None,
        outputs=recent,
    )

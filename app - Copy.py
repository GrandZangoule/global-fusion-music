...
def make_waveform_thumb(audio_path: Path, png_path: Path, width_px=600, height_px=120):
    """
    Robust thumbnail generator. Never throws (swallows errors).
    Requires: matplotlib, soundfile. If anything fails, it silently returns.
    """
    try:
        import matplotlib
        ...
        # Downsample for very long files to speed up plotting
        n = len(wav)
        max_points = 6000  # ~10px per point at 600px width
        if n > max_points:
        ...
        ax.axis("off")
        ax.plot(wav, linewidth=0.8)
        fig.savefig(str(png_path), bbox_inches="tight", pad_inches=0)
        plt.close(fig)
    except Exception:
        # Fail-safe: no thumbnail is not fatal
        return

def recent_gallery_data(limit: int = 20):
    items = []
    try:
        files = sorted(
            (p for p in OUT_DIR.glob("gfm_*") if p.suffix.lower() in AUDIO_EXTS),
        ...
        for p in files:
            try:
                ...
                if not thumb.exists():
                    make_waveform_thumb(p, thumb)
                # ⬇️ only append if we truly have an image
                if not thumb.exists():
                    continue
                items.append({
                    "image": str(thumb),
...
def _play_recent_from_gallery(evt: gr.SelectData):
    """
    On gallery click, load the audio pointed to by metadata['path']
    and send it to your Output player (audio_out expects (sr, ndarray)).
    """
    val = evt.value
    # Newer Gradio returns a dict with 'image', 'label', 'metadata'
    ...
    audio_path = Path(path) if path else (OUT_DIR / label)
    if not audio_path.exists():
        raise gr.Error(f"Audio not found: {audio_path.name}")
    if audio_path.suffix.lower() not in AUDIO_EXTS:
        raise gr.Error("That item is not an audio file.")
...
def _make_waveform_thumb(audio_path: str) -> str:
    """Return PNG path for a small waveform thumbnail; create if missing."""
    p = Path(audio_path)
    thumb = THUMBS / (p.stem + ".png")
    if thumb.exists():
        return str(thumb)
    ...
    try:
        ...
        plt.plot(np.linspace(0, len(wav)/sr, num=min(len(wav), 800)), wav[:800])
        plt.axis("off")
        plt.tight_layout(pad=0)
        plt.savefig(thumb, bbox_inches="tight", pad_inches=0)
        plt.close()
    except Exception:
        # Fallback placeholder if reading fails
        plt.figure(figsize=(3.2,1.2), dpi=100)
...
def _scan_recent(limit: int = 20):
    ...
    for p in files:
        if p.suffix.lower() not in AUDIO_EXTS:
            continue
        thumb = _make_waveform_thumb(str(p))
        RECENT_ITEMS.append((str(p), thumb))
        items.append((thumb, p.name))   # (image_path, caption)
        if len(items) >= limit:
            break
    return items
...
with gr.Blocks(title=APP_NAME, theme=gr.themes.Soft(), fill_height=True) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            file_out = gr.File(
                label="Last generated file",
                file_types=["audio"],
                type="filepath"   # ensures we get a path string back
            )
            ...
            recent = gr.Gallery(
                ...
                columns=5, rows=4, height=600, preview=True
            )
            recent.select(_play_recent_from_gallery, inputs=None, outputs=audio_out)

            refresh_recent_btn = gr.Button("↻ Refresh recent")
            refresh_recent_btn.click(fn=recent_gallery_data, inputs=None, outputs=recent)

            # populate after UI mounts
            demo.load(recent_gallery_data, inputs=None, outputs=recent)
